...
_setPATH:     file format elf32-i386
...
main(int argc, char *argv[])
{
  ...
  61:	83 c4 10             	add    $0x10,%esp
	exit();
  64:	e8 d8 02 00 00       	call   341 <exit>
		printf(2, "Too many Paths, use 10 max\n");
  69:	52                   	push   %edx
  6a:	52                   	push   %edx
  6b:	68 18 08 00 00       	push   $0x818
  70:	6a 02                	push   $0x2
...
set_path( char * p ) {
  ...
  87:	8b 5d 08             	mov    0x8(%ebp),%ebx
	int n = strlen(p);
  8a:	53                   	push   %ebx
  8b:	e8 e0 00 00 00       	call   170 <strlen>
	for (int i = 0 ;  i < n ; i++)
  90:	83 c4 10             	add    $0x10,%esp
  93:	85 c0                	test   %eax,%eax
  95:	7e 18                	jle    af <set_path+0x2f>
  97:	89 da                	mov    %ebx,%edx
  99:	01 d8                	add    %ebx,%eax
  9b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  9f:	90                   	nop
		if (p[i] == ':')
  a0:	80 3a 3a             	cmpb   $0x3a,(%edx)
  a3:	75 03                	jne    a8 <set_path+0x28>
			p[i] = '\0';
  a5:	c6 02 00             	movb   $0x0,(%edx)
  a8:	83 c2 01             	add    $0x1,%edx
	for (int i = 0 ;  i < n ; i++)
  ab:	39 c2                	cmp    %eax,%edx
  ad:	75 f1                	jne    a0 <set_path+0x20>
	printf(1, path[0]);
  ...
  cd:	53                   	push   %ebx
  ce:	6a 01                	push   $0x1
  d0:	e8 db 03 00 00       	call   4b0 <printf>
	printf(1, "\n");
  d5:	58                   	pop    %eax
  d6:	5a                   	pop    %edx
  d7:	68 41 08 00 00       	push   $0x841
  dc:	6a 01                	push   $0x1
...
printf(int fd, const char *fmt, ...)
{
 ...
 634:	eb 1a                	jmp    650 <printf+0x1a0>
          s = "(null)";
 636:	bb 78 08 00 00       	mov    $0x878,%ebx
        while(*s != 0){
